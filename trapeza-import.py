...
import trapeza, trapeza.match, trapeza.formats, tempfile, pickle, io, os, hashlib
from flask import Flask, request, session, make_response, render_template, abort, flash, redirect, url_for
...
def step_one():
    ...
    try:
        ...
        if "input_format" in request.form and request.form.get("input_format") in trapeza.formats.available_input_formats():
        ...
        if master_format == "trapeza":
            ...
            profile = trapeza.match.Profile(source = trapeza.load_source(request.files["profile"], trapeza.get_format(request.files["profile"].filename, "csv"), encoding = input_encoding))
        ...
        flash("An error occurred during loading of input files. Please provide files in a format that Trapeza understands and ensure you choose the correct text encoding.")
    ...
    try:
        ...
        if request.form.get("output_format") and request.form.get("output_format") in trapeza.formats.available_output_formats():
...
def step_two():
    ...
    try:
        ...
        trapeza.write_source(output, out_buffer, operation["output_format"], encoding=operation["output_encoding"], line_endings=endings.get(operation["line_endings"]))
